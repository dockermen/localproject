...
logging.basicConfig(format=__log_format,filename="./alink.log")
...
class CustomerThing(object):
    def __init__(self):
        self.__linkkit = linkkit.LinkKit(
            host_name=userinfo["host_name"],
            product_key=userinfo["product_key"],
            device_name=userinfo["device_name"],
            device_secret=userinfo["device_secret"])
        ...
        while True:
            try:
                ...
                if msg == "1":
                    ...
                    self.__linkkit.thing_trigger_event(("Error", event_data))
                elif msg == "2":
                    temperature = int(input("温度:"))
                    prop_data = {
                        "temperature": temperature#,"power_stage": 10
                    }
                    ...
                    result_rtu = getrtu()
                    self.__linkkit.thing_raw_post_data(json.dumps(result_rtu))
                    print(json.dumps(result_rtu))
                else:
                    sys.exit()


if __name__ == "__main__":
    with open('35btZCIOtD87OsHKYQe2.json','r',encoding='utf-8') as file:
